...
app.post('/books', (req, res) => {
  const { avatar, name, price} = req.body;
  connection.query('INSERT INTO book (name,avatar, price) VALUES (?, ?, ?)', [name, avatar, price], (error, results, fields) => {
    if (error) throw error;
...
app.put('/books/:id', (req, res) => {
  const bookId = req.params.id;
  const { avatar, name, price } = req.body;
  connection.query('UPDATE book SET avatar = ?, name = ?, price = ? WHERE id = ?', [avatar, name, price, bookId], (error, results, fields) => {
    if (error) throw error;
